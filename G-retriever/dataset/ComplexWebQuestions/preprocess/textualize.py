...
def step_one():
  ...
  combined_dataset = concatenate_datasets([sample_dataset['train'], sample_dataset['validation'], sample_dataset['test']])
  ...
  for i in tqdm(range(len(combined_dataset))):
      nodes = {}
      edges = []
      for tri in combined_dataset[i]['graph']:
          h, r, t = tri
          h = h.lower()
          t = t.lower()
          if h not in nodes:
...
def generate_split():
    ...
    train_indices = np.arange(len(sample_dataset['train']))
    val_indices = np.arange(len(sample_dataset['validation'])) + len(sample_dataset['train'])
    test_indices = np.arange(len(sample_dataset['test'])) + len(sample_dataset['train']) + len(sample_dataset['validation'])

    # Fix bug: remove the indices of the empty graphs from the val indices
    test_indices = [i for i in val_indices if i != 335]
...
def step_two():
    ...
    questions = [i['question'] for i in combined_dataset]
    ...
    for index in tqdm(range(len(combined_dataset))):
