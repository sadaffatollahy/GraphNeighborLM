...
class ComplexWebQuestions(Dataset):  # This is for PyTorch-compatible datasets
    def __init__(self):
        super().__init__()
...
def preprocess():
    ...
    for index in tqdm(range(len(combined_dataset))):
        ...
        if not os.path.exists(f'{path_nodes}/{index}.csv') or not os.path.exists(f'{path_edges}/{index}.csv'):
            print(f"Node or edge file missing for index {index}")
            continue
        ...
        # چک کردن فایل‌های گراف و بردارها
        if len(nodes) == 0:
            print(f"Empty nodes at index {index}")
            continue
        if len(edges) == 0:
            print(f"Empty edges at index {index}")
            continue
        if q_embs[index] is None or len(q_embs[index]) == 0:
            print(f"Empty embedding at index {index}")
            continue
        ...
        # تبدیل بردار به ۲ بعدی در صورت نیاز
